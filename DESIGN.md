...
| `src/**/*.test.*` | 共有プリセットと `tbs` 解析、サービスワーカーの状態競合、コンテンツUIの再注入を回帰テストで検証する。 |
...
| `zip.ps1` / `zip.sh` | `manifest.json`、ランタイム用の `src/`、`icons/` だけを配布ZIPへ格納し、テストは除外する。 |
...
1. コンテンツスクリプトは現在URLの `tbs` からqdrだけを抽出し、プリセット外のGoogle標準値も含めてパネルの表示状態を決める。
2. プリセット選択時は既存tbsのqdr以外のセグメントを維持した遷移先を組み立てる。
3. 設定更新メッセージをサービスワーカーへ送り、ストレージとルールの直列更新後に遷移する。一時失敗時は一度だけ再送し、応答しない場合もタイムアウト後に一度だけ遷移する。
4. 一回限りモードではDNRルールを持たず、その遷移URLだけへqdrを直接付与する。次の通常検索には自動適用しない。

### Google検索ツールとの後勝ち連携

1. コンテンツスクリプトは検索語と検索タイプ（`q` / `tbm` / `udm`）が同じリンクに限り、リンク先と現在URLのqdrセグメントを比較する。
2. Google側で期間が変わる場合は `qdr=''` と `keepSetting=false` をサービスワーカーへ送り、拡張機能の維持状態を解除してからGoogle側のリンクへ遷移する。
3. 修飾キー付きクリックでは新しいタブ・ウィンドウで開くブラウザ既定動作を維持し、設定解除だけをサービスワーカーへ送る。
4. 画像サイズや並び順など、qdr以外のtbs変更は期間変更として扱わない。
...
- 保存とDNR適用では `VALID_QDR_VALUES` のqdrだけを受理し、サービスワーカーは送信元拡張機能IDも検証する。URL由来のプリセット外qdrは表示するが維持対象にしない。
- DNRルールの削除と追加は1回の `updateDynamicRules` で行い、ルールが一時的に空になる期間を作らない。
- 設定書き込みとDNR更新は同じPromiseキューで直列化し、ストレージへ保存した最新値からルールを構築する。前段の失敗は後続更新を停止させない。
...
- GoogleのDOM更新で注入先が差し替わった場合は、破棄UIの保留処理を取り消して新しい注入先へパネルを一度だけ再注入する。
- 配布ZIPにテスト、開発用依存、生成スクリプト、ストア素材を含めない。
