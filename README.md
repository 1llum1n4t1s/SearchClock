...
- **期間指定の固定化** — 検索結果上のパネルまたはポップアップから期間を選ぶだけで、以降のGoogle検索に自動適用
- **検索前にインターセプト** — `declarativeNetRequest`でURLを書き換えるため二重ローディングなし
- **後勝ち連携** — Google検索ツールで期間を変更すると拡張機能は自動オフ。拡張機能で変更するとGoogle側をクリア。常に最後の操作が優先
- **インラインパネル** — Google検索結果の上部にパネルを表示。ワンクリックで即再検索
